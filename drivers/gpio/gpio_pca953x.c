...
#define GPIO_PCA953X_INIT(node_id)						\
	static const struct pca953x_config pca953x_cfg_##node_id = {		\
		.i2c = I2C_DT_SPEC_GET(node_id),					\
		.common = {							\
			.port_pin_mask = GPIO_PORT_PIN_MASK_FROM_DT_NODE(node_id), \
		},								\
		.interrupt_enabled = DT_NODE_HAS_PROP(node_id, nint_gpios),	\
		.gpio_int = GPIO_DT_SPEC_GET_OR(node_id, nint_gpios, {0}),	\
		.interrupt_mask = DT_PROP_OR(node_id, interrupt_mask, -1),	\
		.input_latch = DT_PROP_OR(node_id, input_latch, -1),		\
	};									\
										\
	static struct pca953x_drv_data pca953x_drvdata_##node_id = {		\
		.lock = Z_SEM_INITIALIZER(pca953x_drvdata_##node_id.lock, 1, 1), \
	...
	DEVICE_DT_DEFINE(node_id,						\
		gpio_pca953x_init,						\
		NULL,								\
		&pca953x_drvdata_##node_id,					\
		&pca953x_cfg_##node_id,					\
		POST_KERNEL,							\
